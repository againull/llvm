...
const ur_event_handle_t &event_impl::getHandleConstRef() const {
  return MEvent;
}
// ur_event_handle_t &event_impl::getHandleRef() { return MEvent; }
ur_event_handle_t event_impl::getHandle() const {
  std::shared_lock<std::shared_mutex> Lock(MHandleMutex);
  return MEvent;
}

void event_impl::setHandle(const ur_event_handle_t UREvent) {
  std::unique_lock<std::shared_mutex> Lock(MHandleMutex);
  MEvent = UREvent;
}
...
void *event_impl::instrumentationProlog(std::string &Name, int32_t StreamID,
  ...
  Name = SH.nameWithAddress<ur_event_handle_t>("event.wait",
                                               this->getHandleConstRef());
...
template <> uint32_t event_impl::get_info<info::event::reference_count>() {
  auto Handle = this->getHandle();
  if (!MIsHostEvent && Handle) {
    return get_event_info<info::event::reference_count>(this->getHandle(),
                                                        this->getPlugin());
  }
  return 0;
}
...
ur_native_handle_t event_impl::getNative() {
  ...
  if (MContext->getBackend() == backend::opencl)
    Plugin->call(urEventRetain, getHandle());
  ur_native_handle_t OutHandle;
  Plugin->call(urEventGetNativeHandle, getHandle(), &OutHandle);
  return OutHandle;
}
