...
at which the tag command completes on the device.
The event's `info::event_profiling::command_start` timestamp reflects the time
that the tag command starts executing on the device.
...
[_Note:_ The `command_submit` timestamp is defined to equal the GPU completion
time (same as `command_end`), rather than the host-side call time.
This avoids a costly host–device clock synchronization query per call, which
would introduce tens of microseconds of overhead per tag on Xe2/BMG hardware.
For the primary use case of `submit_profiling_tag` (measuring elapsed GPU time
between two tags), the `command_submit` value is irrelevant; only `command_end`
(or `command_start`) is used in the subtraction.
All timestamps are guaranteed to share the same time base as device timestamps
from commands submitted to the same queue.
_{endnote}_]
