...
which the tag operation completes on the device (i.e. when the event enters the
"complete" state as observed on the GPU).
The event's `info::event_profiling::command_start` timestamp reflects the time
that the tag operation starts executing on the device.
Implementations are encouraged to set the `command_start` timestamp to the same
value as the `command_end` timestamp.
The event's `info::event_profiling::command_submit` timestamp reflects the time
at which the tag operation completes on the device.
Implementations are encouraged to set the `command_submit` timestamp to the same
value as the `command_end` timestamp.

[_Note:_ The `command_submit` timestamp for `enqueue_signal_event` is defined to
equal the GPU completion time (same as `command_end`), rather than the host-side
submission time.
This avoids a costly host–device clock synchronization query
(`zeDeviceGetGlobalTimestamps` or equivalent) on every call, which would
introduce tens of microseconds of overhead per tag operation.
The primary use case for the `command_submit` timestamp is measuring the gap
between submission and start of execution, which is meaningful for commands that
queue behind other work.
For a tag operation whose whole purpose is to record a GPU-side point in time,
the submission-to-start gap is not meaningful, so the performance benefit of
using the GPU-written timestamp outweighs the semantic fidelity of a separate
host-captured submission time.
All timestamps (`command_submit`, `command_start`, `command_end`) are guaranteed
to share the same time base as device timestamps from commands submitted to the
same queue.
_{endnote}_]
