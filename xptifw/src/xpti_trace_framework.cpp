...
public:
  ...
  xpti::stream_detail_level_t querySubscribersForStreamDetailLevel(const char *stream_name) {
    ...
    for (const auto &handle_entry : MHandleLUT) {
      ...
      xpti::stream_detail_level_t sub_level = xpti::stream_detail_level_t::XPTI_STREAM_DETAIL_LEVEL_NORMAL;
...
public:
  ...
  xpti::result_t initializeStream(const char *Stream, uint32_t MajorRevision,
    ...
    recomputeEffectiveStreamDetailLevel(stream_id, Stream);
  ...
  /// @brief Recomputes effective detail level by querying all subscribers.
  ///
  ...
  ///
  /// @param stream The stream ID to recompute.
...
private:
  ...
  /// Mutex protecting detail level updates during stream initialization
